/*
 * Jakefile
 * Cappuccino-Kitchen-Sink
 *
 * Created by You on March 18, 2014.
...
var ENV = require("system").env,
    ...
    OS = require("os"),
    projectName = "CappuccinoKitchenSink";
...
app (projectName, function(task)
{
    ...
    task.setBuildIntermediatesPath(FILE.join("Build", "CappuccinoKitchenSink.build", configuration));
    task.setBuildPath(FILE.join("Build", configuration));

    task.setProductName("Cappuccino-Kitchen-Sink");
    task.setIdentifier("com.yourcompany.CappuccinoKitchenSink");
    task.setVersion("1.0");
    task.setAuthor("Your Company");
    task.setEmail("feedback @nospam@ yourcompany.com");
    task.setSummary("Cappuccino-Kitchen-Sink");
...
task ("desktop", ["release"], function()
{
    ...
    require("cappuccino/nativehost").buildNativeHost(FILE.join("Build", "Release", projectName), FILE.join("Build", "Desktop", projectName, "CappuccinoKitchenSink.app"));
    printResults("Desktop")
});

task ("run-desktop", ["desktop"], function()
{
    OS.system([FILE.join("Build", "Desktop", projectName, "CappuccinoKitchenSink.app", "Contents", "MacOS", "NativeHost"), "-i"]);
});
